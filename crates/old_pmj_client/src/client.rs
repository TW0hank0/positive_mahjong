...
impl Client {
    ...
    pub fn view(&self) -> Element<'_, UIMessage, Theme, Renderer> {
        let mut layout: Column<'_, UIMessage, Theme, Renderer> = Column::new()
        ...
        match self.current_scene {
            ClientScenes::Home => {
                ...
                // 訊息顯示
                {
                    ...
                    layout_home =
                        layout_home.push(container(msg_area).style(|theme: &theme::Theme| {
                            let ex_palette = theme.extended_palette();
                            let mut style = container::Style::default();
                            style.background = Some(iced_core::Background::Color(
                                ex_palette.secondary.base.color,
                            ));
                            style.border = Border {
                                color: ex_palette.secondary.strong.color,
                                width: 2.5,
                                radius: Radius::new(Pixels::from(8)),
                            };
                            style
                        }));
                }
                ...
                if self.status_home.try_connecting_server {
                    ...
                    content_column = content_column
                        .push(
                            text("連線中...")
                                .size(28)
                                .style(move |_theme: &theme::Theme| text::Style {
                                    color: Some(ex_palette.secondary.base.text),
                                }),
                        )
                        .spacing(2);
                    let content = container(container(content_column).style(move |_theme| {
                        let mut style = container::Style::default();
                        style = style.background(iced_core::Background::Color(
                    ...
                    .style(move |_theme| {
                        let mut style = container::Style::default();
                        style = style.background(iced_core::Background::Color(Color::TRANSPARENT));
                        style = style.border(Border::default().width(0));
                        style
                    });
                    layout_home =
                        iced_widget::column([stack([layout_home.into(), content.into()]).into()]);
                }
