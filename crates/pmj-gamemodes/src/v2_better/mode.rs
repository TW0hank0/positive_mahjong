...
use crate::v2_better::shared::{
    self as mode_shared, PMJCard, PMJCardFlowerType, PMJCardType, PMJCardWordsType, PMJPlayer,
    PlayerGameActions,
};
    ...
    fn game_loop(&mut self) {
        ...
        'game: loop {
                                        ...
                                        match cgm {
                                            mode_shared::ClientGameMsg::Pga(pga) => {
                                                ...
                                                match pga {
                                                    ...
                                                    PlayerGameActions::ExposedKong {
                                                        with: _,
                                                        kong_card: _,
                                                    } => {
                                                        if &last_turn_player_id == player_id
                                                            && last_action.is_throw_card()
                                                        {
                                                            warn!("不允許的行為！");
                                                        } else {
                                                            current_turn_player_id = *player_id;
                                                    ...
                                                    PlayerGameActions::ConcealedKong {
                                                        with: _,
                                                        kong_card: _,
                                                    } => {
                                                        if &last_turn_player_id == player_id
                                                            && last_action.is_get_card()
                                                        {
                                                            current_turn_player_id = *player_id;
                                                            current_action = pga;
                                                            break 'wait_action;
            ...
            match current_action {
                PlayerGameActions::ExposedKong {
                    with: (ref with1, ref with2, ref with3),
                    ref kong_card,
                } => {
                    ...
                    let with1_card_num = match with1.card_type {
                        mode_shared::PMJCardType::Dots(num) => num,
                        mode_shared::PMJCardType::Line(num) => num,
                        mode_shared::PMJCardType::TenThousand(num) => num,
                        mode_shared::PMJCardType::Flower(ref _flower_type) => {
                            panic!("????: 上個 if 應該處理！");
                        }
                        mode_shared::PMJCardType::Words(ref word_type) => match word_type {
                            ...
                            mode_shared::PMJCardWordsType::West => 6,
                    ...
                    if with1_card_num == with2_card_num
                        && with2_card_num == with3_card_num
                        && with3_card_num == kong_card_num
                    {
                                ...
                                let msg =
                                    serde_json::to_string(&mode_shared::ServerMessage::GameMsg(
                                        mode_shared::ServerGameMsg::PlayerAction(
                                            player.player_id,
                                            PlayerGameActions::ExposedKong {
                                                with: (with1.clone(), with2.clone(), with3.clone()),
                                                kong_card: kong_card.clone(),
                                            },
                                        ),
                                    ))
                                    .unwrap();
                                ...
                                let msg =
                                    serde_json::to_string(&mode_shared::ServerMessage::GameMsg(
                                        mode_shared::ServerGameMsg::PlayerAction(
                                            player.player_id,
                                            PlayerGameActions::Triplet {
                                                with: (with1.clone(), with2.clone()),
                                                triplet_card: triplet_card.clone(),
                                            },
                                        ),
                                    ))
                                    .unwrap();
                ...
                PlayerGameActions::Eat {
                        ...
                        && (card_nums.get(2).unwrap() == &(card_nums.get(1).unwrap() + 1)))
                    {
                        ...
                        let player = self
                            ...
                            .unwrap();
                        player.player_used_cards.push((
                            vec![with1.clone(), with2.clone(), eat_card.clone()],
                            mode_shared::GameActions::Eat,
                        ));
                        for card in [with1.clone(), with2.clone()] {
                            let mut cindex = 0;
                            loop {
                ...
                PlayerGameActions::ThrowCard(ref want_throw_card_ref) => {
                    ...
                    if player.player_hand_cards.contains(&want_throw_card) {
                        ...
                        'find_index: loop {
                            if &want_throw_card == player.player_hand_cards.get(card_index).unwrap()
                            {
                                break 'find_index;
                            } else {
                                card_index += 1;
